...
class NCFTrainer:
    def __init__(self, ui_matrix, epochs, batch_size, latent_dim=32, device=None, clip_norm=100.0, sigma=0.01):
        self.ui_matrix = ui_matrix
        self.epochs = epochs
    ...
    def add_dp_noise_to_weights(self):
        for param in self.ncf.parameters():
            noise = torch.normal(0, self.sigma, size=param.data.size(), device=param.data.device)
    ...
    def train_batch(self, x, y, optimizer):
        ...
        self.add_dp_noise_to_weights()  # Apply DP noise after optimizer step
        optimizer.step()
        optimizer.zero_grad()
