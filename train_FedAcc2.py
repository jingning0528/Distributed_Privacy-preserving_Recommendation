...
def calculate_model_difference(local_model, server_model):
...
def apply_model_differences(server_model, model_differences, num_models):
    server_dict = server_model.state_dict()
...
def federate(utils):
    ...
    for client_model in client_models:
        diff = calculate_model_difference(client_model['model'], server_model)
        model_differences.append(diff)

    apply_model_differences(server_model, model_differences, len(client_models))
...
if __name__ == '__main__':
    ...
    fncf = FederatedNCF(dataloader.ratings, num_clients=50, user_per_client_range=[1, 10], mode="ncf",
                        aggregation_epochs=20, local_epochs=10, batch_size=128)
    fncf.train()