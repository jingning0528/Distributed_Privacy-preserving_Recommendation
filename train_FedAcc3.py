...
def calculate_model_difference(local_model, server_model, momentum=None, previous_diff=None):
    local_dict = local_model.state_dict()
    server_dict = server_model.state_dict()
    difference = {k: local_dict[k] - server_dict[k] for k in server_dict.keys()}
    if momentum is not None and previous_diff is not None:
        difference = {k: difference[k] + momentum * previous_diff[k] for k in difference.keys()}
    return difference


def apply_model_differences(server_model, model_differences, num_models, noise_scale=0):
    server_dict = server_model.state_dict()
    for diff in model_differences:
        if noise_scale > 0:
            diff = {k: v + torch.normal(0, noise_scale, v.shape) for k, v in diff.items()}
        for k, v in diff.items():
            server_dict[k] += v / num_models
    server_model.load_state_dict(server_dict)


def federate(utils, previous_diffs, momentum, noise_scale=0):
    ...
    for i, client_model in enumerate(client_models):
        previous_diff = previous_diffs[i] if previous_diffs else None
        diff = calculate_model_difference(client_model['model'], server_model, momentum, previous_diff)
        model_differences.append(diff)

    apply_model_differences(server_model, model_differences, len(client_models), noise_scale)
...
class FederatedNCF:
    def __init__(self, ui_matrix, num_clients=50, user_per_client_range=[1, 5], mode="ncf", aggregation_epochs=50,
                 local_epochs=10, batch_size=128, latent_dim=32, seed=0, momentum=0.9, noise_scale=0.1):
        random.seed(seed)
        ...
        self.previous_diffs = [None] * self.num_clients
    ...
    def train(self):
        ...
        for epoch in range(self.aggregation_epochs):
            ...
            for client in self.clients:
                client.ncf.to(device)  # Ensure each client's model is on the GPU
                client.ncf.load_server_weights(server_model)

            self.single_round(epoch=epoch, first_time=first_time)
            first_time = False
            self.extract_item_models()
            self.previous_diffs = federate(self.utils, self.previous_diffs, self.momentum, self.noise_scale)


if __name__ == '__main__':
    dataloader = MovielensDatasetLoader()
    fncf = FederatedNCF(dataloader.ratings, num_clients=50, user_per_client_range=[1, 10], mode="ncf",
                        aggregation_epochs=50, local_epochs=10, batch_size=128, momentum=0.9, noise_scale=0.1)
    fncf.train()
