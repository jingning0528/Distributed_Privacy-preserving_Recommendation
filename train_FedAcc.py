...
def federate(utils):
    ...
    server_model = utils.get_previous_federated_model().to('cpu').eval()  # Ensure model is on CPU and in eval mode

    if len(client_models) == 0:
        utils.save_federated_model(server_model)
        return

    # Create a parameter list from the server model, assuming it's not a TorchScript model
    param_list = [param for param in server_model.parameters() if param.requires_grad]

    # Setup the optimizer with the parameter list
    optimizer = torch.optim.LBFGS(param_list, lr=1, max_iter=20, history_size=10)
    ...
    def closure():
        ...
        for k in server_new_dict.keys():
            server_new_dict[k] /= len(client_models)
        server_model.load_state_dict(server_new_dict)

        # Dummy loss function, you need to replace this with your actual loss computation
...
class FederatedNCF:
    ...
    def train(self):
        ...
        for epoch in range(self.aggregation_epochs):
            ...
            #federate(self.utils)
            federate(self.utils, server_lr=0.01)
