...
class FederatedNCF:
    def __init__(self, ui_matrix, num_clients=50, user_per_client_range=[1, 5], mode="ncf", aggregation_epochs=50,
                 local_epochs=10, batch_size=128, latent_dim=32, seed=0, sigma=1.0, clip_norm=1.0):
        random.seed(seed)
    ...
    def add_gaussian_noise(self, model):
        for param in model.parameters():
            if param.grad is not None:
                noise = torch.normal(0, self.sigma, param.shape, device=param.device)
                param.grad += noise
    ...
    def single_round(self, epoch=0, first_time=False):
        ...
        for client_id, client in bar:
            ...
            # Clip gradients
            self.clip_gradients(client.ncf)
            # Add Gaussian noise
            self.add_gaussian_noise(client.ncf)
...
if __name__ == '__main__':
    ...
    fncf = FederatedNCF(dataloader.ratings, num_clients=50, user_per_client_range=[1, 10], mode="ncf",
                        aggregation_epochs=50, local_epochs=10, batch_size=128, sigma=1, clip_norm=1.0)
    fncf.train()
